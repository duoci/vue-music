<template>
  <div class="musicDetailCard" :class="[isMusicDetailCardShow ? '' : 'hide', backgroundMode ? '' : 'glassCard']">
      ...
      <div class="top">
        <div class="left">
          <div class="discContainer">
            <div class="needle" :class="isPlay ? 'needleRotate' : ''" ref="needle">
              <img src="~assets/img/MusicDetailCard/needle.png" alt="" />
            </div>
            <!-- 通过音乐的加载时差删除discAnimation类名再添加,达到重置动画的效果 -->
            <div class="disc" :class="[isPlay ? '' : 'pause', isMusicLoad ? '' : 'discAnimation']" ref="disc">
  ...
  export default {
    ...
    watch: {
      isMusicDetailCardShow(isMusicDetailCardShow) {
        ...
        if (isMusicDetailCardShow && !this.comment.comments && this.musicId !== '') {
      ...
      musicId(musicId) {
        ...
        // 更新当前歌曲信息
        this.musicInfo = this.musicList[this.currentIndex];
        this.comment = {};
        // 优化性能,仅在卡片展示时才发送请求
    ...
    computed: {
      ...mapState(['isPlay', 'isMusicLoad', 'musicId', 'isMusicDetailCardShow', 'currentIndex', 'musicList']),
    },
    methods: {
      ...mapMutations(['changeMusicDetailCardState']),
      // 请求并处理歌词数据
      async getLyric(id) {
        const res = await this.get('/lyric', { id });
      ...
      async getMusicComment(id, type) {
        ...
        if (type === 'changePage') {
          const musicDetailCardContainer = document.querySelector('.musicDetailCardContainer');
        ...
        // 获取当前歌曲评论数据
        const res = await this.get('/comment/music', {
          id,
      ...
      commentPageChange(page) {
        ...
        // 评论页数发生变化，调用获取评论函数，通过'changPage'的类型实现获取特定的评论页数数据
